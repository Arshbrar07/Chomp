...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!env.SHRINK_API) {
      return Response.json(
        { status: 'error', error: 'Live data binding unavailable' },
        { status: 503 }
      );
    }
    ...
    const cacheKey = new Request(`https://chomp-api-cache.internal/chomp-mint-v8${url.pathname}`);
    const cached = await cache.match(cacheKey);
    if (cached) return cached;

    try {
      const upstream = new Request(`https://shrink.internal${url.pathname}`, {
        method: 'GET',
      ...
      console.error(JSON.stringify({
        message: 'CHOMP live-data proxy failed',
        path: url.pathname,
        error: error instanceof Error ? error.message : String(error)
      }));
      return Response.json(
        { status: 'error', error: 'Live data temporarily unavailable' },
